...
{jQuery("body").data("sf_results",null);var loading="<li class=\"sf_lnk sf_more sf_selected\">"+"<a id=\"sf_loading\" href=\""+options.searchUrl.replace('%s',encodeURI(jQuery(elem).val()))+"\">"+"</a>"+"</li>";jQuery("#sf_val").html("<ul>"+loading+"</ul>");var pos=this.bounds(elem,options);if(!pos){jQuery("#sf_sb").hide();return!1}
...
else{jQuery("#sf_val").html("<ul>"+m+"</ul>")}
...
{jQuery("#sf_sb").focus()}
var s_item=null;var after_s_item=null;var s_sel=!1;var all_items=jQuery("#sf_val li.sf_lnk");var s_found=!1;event.stopPropagation();event.preventDefault();for(var i=0;i<all_items.length;i++)
{if(jQuery(all_items[i]).attr("class").indexOf("sf_selected")>=0&&s_found==!1)
{s_sel=!0;if(i<all_items.length-1&&event.keyCode=="40")
{jQuery(all_items[i]).attr("class",jQuery(all_items[i]).attr("class").replace(" sf_selected",""));jQuery(all_items[i+1]).attr("class",jQuery(all_items[i+1]).attr("class")+" sf_selected");i=i+1;s_found=!0}
else if(i>0&&event.keyCode=="38")
{jQuery(all_items[i]).attr("class",jQuery(all_items[i]).attr("class").replace(" sf_selected",""));jQuery(all_items[i-1]).attr("class",jQuery(all_items[i-1]).attr("class")+" sf_selected");i=i+1;s_found=!0}}
else{jQuery(all_items[i]).attr("class",jQuery(all_items[i]).attr("class").replace(" sf_selected",""))}}
if(s_sel==!1)
...
return!1}
else{if(d.object.options.callback){d.object.options.callback(this)}
